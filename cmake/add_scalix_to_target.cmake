...
if (DEFINED "AdaptiveCpp_DIR")
    ...
    set(SCALIX_EXTRA_SYCL_ARGS "-fsycl-targets=${SCALIX_SYCL_TARGETS}" ${SCALIX_EXTRA_SYCL_ARGS})
endif ()

function(scalix_cxx_flags_for_target target)
    ...
    if (DEFINED "SCALIX_BASE_SYCL_FLAGS")
        target_compile_options(${target} BEFORE PUBLIC ${SCALIX_BASE_SYCL_FLAGS})
        target_link_options(${target} BEFORE PUBLIC ${SCALIX_BASE_SYCL_FLAGS})
    endif ()
endfunction()

function(scalix_target_common target)
...
function(scalix_add_acpp_to_target target)
    if (DEFINED "AdaptiveCpp_DIR")
...
function(add_scalix_to_library target)
    scalix_cxx_flags_for_target(${target})
    ...
    scalix_target_common(${target})

    if (DEFINED "SCALIX_CXX_CLANG_TIDY")
        scalix_add_clang_tidy_to_library(${target} ${SCALIX_CXX_CLANG_TIDY})
    endif ()

    scalix_add_acpp_to_target(${target})
endfunction()

function(add_scalix_to_executable target)
    scalix_cxx_flags_for_target(${target})
    scalix_target_common(${target})
